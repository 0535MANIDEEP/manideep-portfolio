    ...
    <section id="work" className="py-16 sm:py-20 px-4 border-t border-[#e7e5e4]">
      <div className="mx-auto max-w-3xl">
        ...
        <p className="mt-2 text-sm text-[#78716c]">
          Two deployed projects with source code available to review.
        </p>
        ...
        <div className="mt-8 space-y-8">
          {portfolio.projects.map((project) => (
            <article
              key={project.name}
              className="rounded-lg border border-[#e7e5e4] p-5 sm:p-6"
            >
              ...
              <div className="mt-4 flex flex-wrap gap-2">
                {project.stack.map((tech) => (
                  <span
                    key={tech}
                    className="inline-block rounded-md bg-[#f5f5f4] px-2 py-0.5 text-xs font-medium text-[#78716c]"
                  >
                    {tech}
                  </span>
                ))}
              ...
              <div className="mt-4">
                ...
                <ul className="space-y-1">
                  {project.features.map((f) => (
                    <li
                      key={f}
                      className="text-sm text-[#1c1917] flex items-start gap-2"
                    >
              ...
              <div className="mt-4">
                ...
                <ul className="space-y-1">
                  {project.engineering.map((e) => (
                    <li
                      key={e}
                      className="text-sm text-[#78716c] flex items-start gap-2"
                    >
              ...
              <div className="mt-5 flex flex-wrap gap-3">
                <a
                  ...
                  className="inline-flex items-center rounded-md bg-[#1c1917] px-3.5 py-1.5 text-sm font-medium text-[#fafaf9] hover:bg-[#44403c] transition-colors"
                >
                  Live demo
                ...
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="inline-flex items-center rounded-md border border-[#e7e5e4] px-3.5 py-1.5 text-sm font-medium text-[#1c1917] hover:bg-[#f5f5f4] transition-colors"
                >
                  View source
                  <svg className="ml-1.5 h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                    <path strokeLinecap="round" strokeLinejoin="round" d="M14 5l7 7m0 0l-7 7m7-7H3" />
        ...
        <div className="mt-10 rounded-lg border border-dashed border-[#d6d3d1] p-5">
          <h3 className="text-base font-semibold text-[#1c1917]">
            {portfolio.additionalWork.name}
            <span className="ml-2 text-sm font-normal text-[#78716c]">
              {portfolio.additionalWork.subtitle}
            </span>
          </h3>
          <p className="mt-2 text-sm text-[#78716c] leading-relaxed">
            {portfolio.additionalWork.description}
          </p>
          <div className="mt-4">
            <a
              href={portfolio.additionalWork.github}
              target="_blank"
              rel="noopener noreferrer"
              className="inline-flex items-center rounded-md border border-[#e7e5e4] px-3.5 py-1.5 text-sm font-medium text-[#1c1917] hover:bg-[#f5f5f4] transition-colors"
            >
